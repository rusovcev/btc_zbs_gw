...
from zbs_gateway.common import MappingNotFoundForCoinAddress, \
    MultipleGatewayReceiversError, Injectable, GATEWAY_OWNER_ADDRESS, GATEWAY_COIN_ADDRESS, \
    ONLY_ONE_TRANSACTION_RECEIVER, GATEWAY_ZBS_ADDRESS, GATEWAY_ZBSPY_ADDRESS
from zbs_gateway.model import Transaction, TransactionReceiver, \
    TransactionAttempt, TransactionAttemptReceiver, TransactionAttemptList, \
    AttemptListTrigger, TransactionSender
from zbs_gateway.storage import MapStorage, TransactionAttemptListStorage
...
@Injectable(deps=[
    TransactionAttemptListService, GATEWAY_COIN_ADDRESS, MapStorage, Logger, GATEWAY_ZBS_ADDRESS,
    GATEWAY_OWNER_ADDRESS, FeeServiceConverterProxyImpl, ONLY_ONE_TRANSACTION_RECEIVER, GATEWAY_ZBSPY_ADDRESS,
...
class CoinTransactionConsumerImpl(TransactionConsumer):
    ...
    def __init__(self, attempt_service: TransactionAttemptListService, gateway_coin_address: str,
                 map_storage: MapStorage, logger: Logger, gateway_zbs_address: str, gateway_owner_address: str,
                 fee_service: FeeService, only_one_transaction_receiver: bool, gateway_zbspy_address: zbspy.Address,
    ...
    def _handle_transaction_receiver(self,
        ...
        zbs_fee = cast(int, self._fee_service.get_zbs_fee())
        ...
        receiver_zbs_address = self._map_storage.get_zbs_address_by_coin_address(gateway_managed_address)
        ...
        if receiver_zbs_address is None:
        ...
        if attempt_list is None:
            ...
            attempts.append(
                TransactionAttempt(
                    sender=self._gateway_zbspy_address.address,
                    fee=zbs_fee,
                    currency="zbs",
                    receivers=[TransactionAttemptReceiver(address=receiver_zbs_address, amount=amount_after_fees)]))
