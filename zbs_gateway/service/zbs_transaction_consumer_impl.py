...
from zbs_gateway.common import MultipleGatewayReceiversError, Injectable, GATEWAY_OWNER_ADDRESS, \
    GATEWAY_COIN_ADDRESS_SECRET, ONLY_ONE_TRANSACTION_RECEIVER, GATEWAY_ZBS_ADDRESS_SECRET, GATEWAY_ZBSPY_ADDRESS, \
    ZBS_CHAIN_QUERY_SERVICE_CONVERTER_PROXY
from zbs_gateway.model import TransactionReceiver, \
...
@Injectable(deps=[
    GATEWAY_ZBS_ADDRESS_SECRET, ZBS_CHAIN_QUERY_SERVICE_CONVERTER_PROXY, Logger, GATEWAY_OWNER_ADDRESS,
    GATEWAY_COIN_ADDRESS_SECRET, FeeServiceConverterProxyImpl, ONLY_ONE_TRANSACTION_RECEIVER,
    TransactionAttemptListService, GATEWAY_ZBSPY_ADDRESS, TransactionAttemptListStorage,
    COIN_ADDRESS_VALIDATION_SERVICE
])
class ZbsTransactionConsumerImpl(TransactionConsumer):
    """
    Takes the received transactions from Waves and issues the resulting transactions
    in the custom cryptocurrency.
    """

    def __init__(self, gateway_zbs_address_secret: KeyPair, zbs_chain_query_service: ChainQueryService,
                 ...
                 attempt_service: TransactionAttemptListService, gateway_zbspy_address: zbspy.Address,
    ...
    def _filter_receivers(self, transaction: Transaction) -> List[int]:
        ...
        for i in range(0, len(transaction.receivers)):
            if self._gateway_zbspy_address.address == transaction.receivers[i].address:
                attempt_list = self._attempt_list_storage.find_by_trigger(
                    AttemptListTrigger(tx=transaction.tx, currency="zbs", receiver=i))
    ...
    def _handle_receiver(self,
        ...
        coin_receiver = self._zbs_chain_query_service.get_coin_receiver_address_from_transaction(tx)
