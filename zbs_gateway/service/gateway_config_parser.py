...
from zbs_gateway.common import Injectable, KeyPair, InvalidConfigError
from zbs_gateway.model import GatewayConfigFile
...
class GatewayConfigParser(object):
    ...
    def _parse_node_section(self, config_parser: ConfigParser, parsed_config: GatewayConfigFile) -> None:
        parsed_config.zbs_node = config_parser.get('node', 'zbs')
    ...
    def _parse_gateway_address_section(self, config_parser: ConfigParser, parsed_config: GatewayConfigFile):
        ...
        gateway_zbs_address_public_key = config_parser.get('gateway_address', 'zbs_public_key')
        gateway_zbs_address_private_key = config_parser.get('gateway_address', 'zbs_private_key')
        ...
        parsed_config.gateway_zbs_address_secret = KeyPair(
            public=gateway_zbs_address_public_key, secret=gateway_zbs_address_private_key)
    ...
    def _parse_other_section(self, config_parser: ConfigParser, parsed_config: GatewayConfigFile) -> None:
        parsed_config.zbs_chain = config_parser.get('other', 'zbs_chain', fallback="mainnet")
        parsed_config.zbs_chain_id = config_parser.get('other', 'zbs_chain_id', fallback=None)
    ...
    def _assert_not_old_format(self, config_parser: ConfigParser):
        ...
        # assert_option_does_not_exist('gateway_address', 'coin_public_key')
        # assert_option_does_not_exist('gateway_address', 'coin_private_key')
