...
class MappingNotFoundForCoinAddress(Exception):
    ...
    def __init__(self, coin_address: str) -> None:
        super().__init__(
            "The address '" + coin_address + "' has no associated ZBS address, but was expected to have one.")
...
class ZbsPyError(Exception):
    """
    Raised when ZbsPy encountered any error.
    """
    pass


class ZbsAddressInvalidError(Exception):
    """
    Raised when an invalid ZBS address was overhanded.
    """
    pass
...
class ZbsNodeException(Exception):
    """
    Raised when the ZBS node was unable to process the request for some reason.
    """
