...
from zbs_gateway.service import COIN_TRANSACTION_POLLING_SERVICE, \
    ZBS_TRANSACTION_POLLING_SERVICE, ZbsTransactionConsumerImpl, CoinTransactionConsumerImpl, IntegerConverterService
from zbs_gateway.common import Factory, LOGGING_HANDLER_LIST, MANAGED_LOGGER_LIST, POLLING_DELAY_SECONDS, \
    ...
    GATEWAY_COIN_ADDRESS, ZBS_NODE, ZBS_ASSET_ID, ZBS_CHAIN, ONLY_ONE_TRANSACTION_RECEIVER, \
    COIN_TRANSACTION_WEB_LINK, ZBS_TRANSACTION_WEB_LINK, COIN_ADDRESS_WEB_LINK, ZBS_ADDRESS_WEB_LINK, \
    ATTEMPT_LIST_MAX_COMPLETION_TRIES, GATEWAY_ZBS_ADDRESS_SECRET, GATEWAY_ZBS_ADDRESS, NUM_ATTEMPT_LIST_WORKERS, \
    GATEWAY_HOST, GATEWAY_PORT, WALLET_STORAGE_COLLECTION, MAP_STORAGE_COLLECTION, KEY_VALUE_STORAGE_COLLECTION, \
    TRANSACTION_ATTEMPT_LIST_STORAGE_COLLECTION, GATEWAY_ZBSPY_ADDRESS, COIN_CHAIN_QUERY_SERVICE_CONVERTER_PROXY, \
    ZBS_CHAIN_QUERY_SERVICE_CONVERTER_PROXY, FLASK_NAME, COIN_MAX_HANDLE_TRANSACTION_TRIES, \
    ZBS_MAX_HANDLE_TRANSACTION_TRIES, ZBS_LAST_BLOCK_DISTANCE, COIN_LAST_BLOCK_DISTANCE, WEB_PRIMARY_COLOR, \
    ZBS_CHAIN_ID
from zbs_gateway.model import PollingDelayConfig
from zbs_gateway.factory import CoinAddressFactory

import zbs_gateway.common as common
import zbs_gateway.controller as controller
import zbs_gateway.model as model
import zbs_gateway.service as service
import zbs_gateway.storage as storage
import zbs_gateway.factory as factory
from pymongo.database import Database as MongoDatabase  # type: ignore

from zbs_gateway.model import PublicConfiguration
from zbs_gateway.service import COIN_CHAIN_QUERY_SERVICE, COIN_ADDRESS_VALIDATION_SERVICE, COIN_TRANSACTION_SERVICE, COIN_INTEGER_CONVERTER_SERVICE, \
    ASSET_INTEGER_CONVERTER_SERVICE
from zbs_gateway.storage import CoinBlockHeightStorageProxyImpl, ZbsBlockHeightStorageProxyImpl, CoinPollingStateStorageProxyImpl, ZbsPollingStateStorageProxyImpl, PollingStateStorageProxy
...
@Factory(GATEWAY_ZBSPY_ADDRESS, deps=[GATEWAY_ZBS_ADDRESS_SECRET, ZBS_NODE, ZBS_CHAIN], opt_deps=[ZBS_CHAIN_ID])
def gateway_zbspy_address(gateway_zbs_address_secret: model.KeyPair, zbs_node: str, zbs_chain: str, zbs_chain_id: Optional[str] = None):
    """Creates an address instance from the pywaves library that represents the Gateway waves address."""
    zbspy.setNode(zbs_node, zbs_chain, zbs_chain_id)
    return zbspy.Address(gateway_zbs_address_secret.public, privateKey=gateway_zbs_address_secret.secret)
...
@Factory(
    ZBS_TRANSACTION_POLLING_SERVICE,
    deps=[
        ZBS_CHAIN_QUERY_SERVICE_CONVERTER_PROXY, logging.Logger, ZbsBlockHeightStorageProxyImpl,
        ZbsTransactionConsumerImpl, model.PollingDelayConfig, ZbsPollingStateStorageProxyImpl,
        ZBS_MAX_HANDLE_TRANSACTION_TRIES, ZBS_LAST_BLOCK_DISTANCE
    ])
def zbs_transaction_polling_service_factory(zbs_chain_query_service_converter_proxy, logger,
                                              zbs_block_height_storage_proxy, zbs_transaction_consumer,
                                              polling_delay_config, polling_state_storage: PollingStateStorageProxy,
    ...
    return service.TransactionPollingService(
        chain_query_service=zbs_chain_query_service_converter_proxy,
        logger=logger,
        block_height_storage=zbs_block_height_storage_proxy,
        transaction_consumer=zbs_transaction_consumer,
        max_polling_delay_s=polling_delay_config.zbs_max_polling_delay_s,
        min_polling_delay_s=polling_delay_config.zbs_min_polling_delay_s,
        max_handle_transaction_tries=max_handle_transaction_tries,
        polling_state_storage=polling_state_storage,
        last_block_distance=last_block_distance)
...
@Factory(
    PublicConfiguration,
    deps=[
        CUSTOM_CURRENCY_NAME, GATEWAY_ZBS_ADDRESS, GATEWAY_COIN_ADDRESS, ZBS_NODE, ZBS_ASSET_ID,
        ZBS_TRANSACTION_WEB_LINK, ZBS_ADDRESS_WEB_LINK, WEB_PRIMARY_COLOR
    ],
...
def public_configuration_factory(custom_currency_name: str,
    ...
    return model.PublicConfiguration(
        ...
        gateway_zbs_address=gateway_zbs_address,
...
@Factory(ZBS_ADDRESS_WEB_LINK, weak=True, deps=[ZBS_CHAIN])
def zbs_address_web_link_factory(zbs_chain: str):
    """Provides default value for ZBS_ADDRESS_WEB_LINK depending on ZBS_CHAIN."""
    if zbs_chain == 'testnet':
        return 'https://explorer.testnet-0bsnetwork.com/address/{{address}}'
    else:
        return 'https://google.com'


@Factory(ZBS_TRANSACTION_WEB_LINK, weak=True, deps=[ZBS_CHAIN])
def zbs_transaction_web_link_factory(zbs_chain: str):
    """Provides default value for ZBS_TRANSACTION_WEB_LINK depending on ZBS_CHAIN."""
    if zbs_chain == 'testnet':
        return 'https://explorer.testnet-0bsnetwork.com/tx/{{tx}}'
    else:
        return 'https://google.com'
...
class Gateway(object):
    ...
    DEFAULT_FLASK_NAME = 'zbs-gw'
    DEFAULT_CUSTOM_CURRENCY_NAME = 'Custom Currency'
    DEFAULT_POLLING_DELAY = 0
    DEFAULT_ZBS_POLLING_DELAY = 0
    DEFAULT_HOSTNAME = 'localhost'
    DEFAULT_PORT = 5000
    DEFAULT_ZBS_CHAIN = 'mainnet'
    ...
    DEFAULT_ZBS_MAX_HANDLE_TRANSACTION_TRIES = 5
    DEFAULT_COIN_LAST_BLOCK_DISTANCE = 0
    DEFAULT_ZBS_LAST_BLOCK_DISTANCE = 1
    ...
    def __init__(self,
                 ...
                 gateway_zbs_address_secret: model.KeyPair,
                 ...
                 coin_transaction_web_link: Optional[str] = None,
                 zbs_transaction_web_link: Optional[str] = None,
                 coin_address_web_link: Optional[str] = None,
                 zbs_address_web_link: Optional[str] = None,
                 ...
                 zbs_max_handle_transaction_tries: int = DEFAULT_ZBS_MAX_HANDLE_TRANSACTION_TRIES,
                 coin_last_block_distance: int = DEFAULT_COIN_LAST_BLOCK_DISTANCE,
                 zbs_last_block_distance: int = DEFAULT_ZBS_LAST_BLOCK_DISTANCE,
                 web_primary_color: str = DEFAULT_WEB_PRIMARY_COLOR,
                 zbs_chain_id: Optional[str] = None
                 ) -> None:
        """
        ...
        :param gateway_zbs_address_secret:
            Represents the address of the Gateway in ZBS.
        ...
        :param gateway_owner_address:
            ...
            given percentage 'gateway_owner_zbs_fee_percentage'.
        ...
        self._injector.overwrite(ZBS_NODE, zbs_node)
        self._injector.overwrite(ZBS_ASSET_ID, zbs_asset_id)
        self._injector.overwrite(ZBS_CHAIN, zbs_chain)
        ...
        self._injector.overwrite_if_exists(ZBS_TRANSACTION_WEB_LINK, zbs_transaction_web_link)
        self._injector.overwrite_if_exists(COIN_ADDRESS_WEB_LINK, coin_address_web_link)
        self._injector.overwrite_if_exists(ZBS_ADDRESS_WEB_LINK, zbs_address_web_link)
        ...
        self._injector.overwrite(GATEWAY_ZBS_ADDRESS_SECRET, gateway_zbs_address_secret)
        self._injector.overwrite(GATEWAY_ZBS_ADDRESS, gateway_zbs_address_secret.public)
        ...
        self._injector.overwrite(ZBS_MAX_HANDLE_TRANSACTION_TRIES, zbs_max_handle_transaction_tries)
        self._injector.overwrite(COIN_LAST_BLOCK_DISTANCE, coin_last_block_distance)
        self._injector.overwrite(ZBS_LAST_BLOCK_DISTANCE, zbs_last_block_distance)
        self._injector.overwrite(WEB_PRIMARY_COLOR, web_primary_color)
        self._injector.overwrite_if_exists(ZBS_CHAIN_ID, zbs_chain_id)
