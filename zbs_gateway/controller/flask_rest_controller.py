...
from zbs_gateway.common import map_array, ZbsAddressInvalidError, InvalidTransactionIdentifier, Injectable

import flask as flask_module  # type: ignore

from zbs_gateway.model.public_configuration import PublicConfiguration
from zbs_gateway.serializer import TransactionAttemptListSerializer
from zbs_gateway.serializer.public_configuration_serializer import PublicConfigurationSerializer
...
class FlaskRestController(object):
    ...
    def __init__(self, flask: Any, gateway_controller: GatewayController, logger: Logger,
        ...
        self._add_url_rule(
            '/api/v1/coin-address/<zbs_address>',
            'create_address',
            lambda zbs_address: self._create_address(zbs_address),
            methods=['GET'])
        ...
        self._add_url_rule(
            '/api/v1/check/zbs/<tx>', 'check_zbs_tx', lambda tx: self._check_zbs_transaction(tx), methods=['GET'])

        self._add_url_rule(
            '/api/v1/check/coin/<tx>', 'check_coin_tx', lambda tx: self._check_coin_transaction(tx), methods=['GET'])

    def _create_address(self, zbs_address: str) -> str:
        """
        Forwards an create address request to the GatewayController.
        """
        try:
            coin_address = self._gateway_controller.create_address(zbs_address)
        except ZbsAddressInvalidError:
    ...
    def _check_zbs_transaction(self, tx: str) -> str:
        try:
            self._gateway_controller.check_zbs_transaction(tx)
        except InvalidTransactionIdentifier:
            flask_module.abort(400)
